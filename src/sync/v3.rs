            ...
            for (room_id, state_after) in &self.joined_state_after {
                if let Some(room) = join.get_mut(room_id.as_str()) {
                    ...
                    if let Some(obj) = room.as_object_mut() {
                        ...
                        obj.insert(
                            "org.matrix.msc4222.state_after".to_owned(),
                            state_after_obj,
                        );
                    }
                }
...
mod tests {
    ...
    fn test_patch_state_after() {
        let builder = SyncResponseBuilder::new().joined_state_after(vec![
            ("!room:example.com".to_owned(), json!([{"type": "m.room.create"}])),
        ]);
        let mut val = json!({
        ...
        assert!(val["rooms"]["join"]["!room:example.com"]["org.matrix.msc4222.state_after"].is_object());
    }
    ...
    fn test_patch_knock_rooms() {
        let builder =
            SyncResponseBuilder::new().knocked_rooms_json(json!([{"room_id": "!knock:example.com"}]));
