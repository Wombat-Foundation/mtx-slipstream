...
/// Streaming writer for federation responses with `state` and `auth_chain` arrays.
...
impl FederationResponseWriter {
    pub fn with_capacity(estimated_state_pdus: usize, estimated_auth_chain_pdus: usize) -> Self {
        ...
        Self { buf, state_count: 0, auth_chain_count: 0, phase: ResponsePhase::State }
    }
    ...
    pub fn write_state_pdu<T: Serialize>(
        &mut self,
        pdu: &T,
