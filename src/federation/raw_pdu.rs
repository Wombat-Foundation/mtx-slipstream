...
//! 1. **simd-json parsing** — use `simd_json::from_slice` instead of `serde_json::from_slice`
//!    for SIMD-accelerated deserialization of PDUs from the database.
//! 2. **Direct serialization** — serialize `CanonicalJsonObject` directly to bytes,
...
mod tests {
    ...
    fn test_canonical_to_bytes_without() {
        ...
        let bytes =
            canonical_to_bytes_without(&obj, &["unsigned"]).unwrap();
        let parsed: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
        assert_eq!(parsed["event_id"], "$abc");
